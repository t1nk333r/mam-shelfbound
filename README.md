...
- At startup the app checks whether `/downloads` and `/library` sit on the same filesystem and logs a warning when they do not, so a layout that cannot hardlink surfaces before the first import rather than partway through one. In `copy` mode the warning instead notes that files will be copied and use extra disk space.
